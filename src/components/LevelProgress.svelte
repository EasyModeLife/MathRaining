<script lang="ts">
  ...
  <div class="level-progress">
    <div class="level-progress__meta">
      ...
      <div class="level-progress__count-inline"><MathRenderer expr={countExpr} /></div>
      {#if showTimer}
...
<style>
  .level-progress { display:flex;flex-direction:column;gap:.5rem; }
  .level-progress__meta { display:flex;align-items:center;gap:.8rem; }
  ...
  @media (max-width:560px){ .level-progress { gap:.6rem; } .level-progress__label { font-size:.72rem; } }
  @media (max-width:600px){ .level-progress__label { font-size:.68rem; } }
</style>
